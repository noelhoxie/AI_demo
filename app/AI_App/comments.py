...
# Optional: use shared db_connection from repo root (parent of AI_App)
try:
    import sys
...
def get_comments(tab: Optional[str] = None) -> list[dict[str, Any]]:
    ...
    try:
        with conn.cursor() as cur:
            if tab:
                cur.execute(
                    f'SELECT id, tab, content, created_at FROM "{schema}"."{table}" WHERE tab = %s ORDER BY created_at DESC',
                    (tab,),
                )
            else:
                cur.execute(f'SELECT id, tab, content, created_at FROM "{schema}"."{table}" ORDER BY created_at DESC')
            rows = cur.fetchall()
        return [
            {
                "id": r[0],
...
def add_comment(tab: str, content: str) -> Optional[dict[str, Any]]:
    """Insert a comment; return the created row or None."""
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                f'INSERT INTO "{schema}"."{table}" (tab, content) VALUES (%s, %s) RETURNING id, tab, content, created_at',
                (tab.strip(), content.strip()),
            )
            row = cur.fetchone()
        conn.commit()
        if row:
