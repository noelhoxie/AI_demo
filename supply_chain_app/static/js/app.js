...
async function fetchIbp() {
  try {
    const d = await (await fetch('/api/ibp')).json();
...
async function fetchDemand() {
  try {
    const d = await (await fetch('/api/demand')).json();
...
async function diagnoseGenie() {
  ...
  try {
    const res  = await fetch('/api/debug/genie');
    const data = await res.json();
    const lines = [];
